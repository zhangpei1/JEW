...
/**
 * @author zhangpei
 * @date on 17-8-21 上午11:27
 * @package com.new_jew.main.ui.activity
 */

public class SellCarActivity extends BaseActivity implements Popinterface, CheckCarible, SellCarDeleteible {
    ...
    public static String paths = Environment.getExternalStorageDirectory().getAbsolutePath() + "/Jew/sellcar/";//sd路径
    private File file;
...
//上传图片的md
    ...
    protected int setRootView() {
        return R.layout.layout_sell_car;
    }
...
//        bitmap = BitmapCompressUtils.imageZoom(root_paths, 300);
    ...
    void PutData() {
        ...
        RequestParams params = new RequestParams(Api.my_creditor_rights.MY_CREDITOR_RIGHTS + Constants.order_id + "/vehicle_sell/");
        for (int i = 1; i < file_list.size(); i++) {
            stringBuffer.append("," + file_list.get(i));

        }
        Log.e("图片", String.valueOf(stringBuffer));
        params.addBodyParameter("vehicle_sales_voucher", String.valueOf(stringBuffer));
        params.addBodyParameter("amount_sold", amountSold.getText().toString());
        params.addBodyParameter("selling_time", sellingTime.getText().toString());
        params.addBodyParameter("car_buying_side", carBuyingSide.getText().toString());
